...
const KB_DOCS = [
  {
    ...
    tags: ["dink", "control", "kitchen", "consistency", "rate"],
  ...
  },
  {
    ...
    tags: ["rally", "fatigue", "errors", "long", "unforced", "endurance", "shots"],
    content: `Research shows unforced error rates increase sharply after 15-20 shots for intermediate players. Fatigue tolerance drills: (1) 30-ball feeding drill — maintain form past shot 20. (2) Live rally tracking — identify personal error threshold and attack before it. (3) Breathing protocol — exhale on contact to manage tension buildup. Interval training 3×5 min high-intensity drills with 2-min rest improves rally endurance. Tactically, recognize your fatigue threshold and transition to attack mode at shot 14-16 to avoid entering your error zone.`,
  },
  {
    id: "court-positioning",
    title: "Court Positioning & Coverage Principles",
    tags: ["positioning", "court", "coverage", "heatmap", "baseline", "transition", "movement"],
  ...
  {
    id: "left-side-coverage",
    title: "Left-Side Defensive Coverage & Backhand Positioning",
    tags: ["left", "backhand", "coverage", "vulnerability", "gap", "side", "corridor"],
    content: `Left-side court vulnerability is one of the most common patterns in intermediate pickleball. Root causes: (1) Right-hand dominant players default to forehand side. (2) Partner overlap in doubles leaving the left corridor exposed. Key drills: (1) Left-corridor shadow drill — 5 min of lateral movement to left side only. (2) Backhand dink from left position — 50 reps per session. (3) Cross-court recovery drill — recover to left-center after every wide forehand. Target: 40%+ of court time on left half of assigned zone within 4 weeks.`,
  },
  {
    id: "volley-technique",
    title: "Volley Technique, Consistency & Success Rate",
    tags: ["volley", "reflex", "technique", "success", "rate", "NVZ", "punch"],
    content: `Volley success rate of 76-80% is intermediate range. To reach 85%+: (1) Compact swing — minimal backswing for faster reaction time. (2) Ready position — paddle up at chest height between volleys. (3) Punch volley drill — wall volley 2 min/day for muscle memory. (4) Speed volley exchanges with partner — 3 sets of 20 reps. Expected progression: 76% → 82% in 6 weeks, 82% → 85% in 12 weeks with 4-5 sessions/week. Key metric: track volley errors (unforced vs forced) to isolate whether the issue is technique or reaction speed.`,
  },
  {
    id: "dupr-progression",
    title: "DUPR Rating Progression & Skill Benchmarks",
    tags: ["DUPR", "rating", "progression", "skill", "target", "improvement", "milestone"],
    content: `DUPR 4.0 represents solid intermediate play with consistent 3rd-shot drops, kitchen control, and rally management. Key progression levers: (1) Eliminating unforced errors — highest DUPR impact per improvement unit. (2) Increasing 3rd-shot drop percentage to 70%+. (3) Maintaining 80%+ dink success rate in competitive play. (4) Reducing double-fault rate. Expected timeline: 12-16 weeks with structured practice at 4-5 sessions/week. DUPR responds to win/loss outcomes — focus on match quality over drilling quantity. Consistency in close games has 2× the rating impact of blowout wins.`,
  },
  {
    id: "matchup-strategy",
    title: "Opponent Matchup Strategy for Defensive Players",
    tags: ["matchup", "opponent", "strategy", "defensive", "aggressive", "style", "win", "queue"],
    content: `Defensive and control-style players perform best against aggressive power opponents. Win rate analysis: vs. aggressive players, advanced defensive players achieve 68-74% projected win rates. Strategy: (1) Let opponents attack first — use their pace to redirect, not generate. (2) Extend rallies past their comfort zone at 15+ shots — power players fatigue faster. (3) Target their backhand with dinks — most power players have weaker backhand touch control. (4) Open play queue selection: target sessions with higher-rated aggressive opponents for maximum improvement stimulus and favorable style matchup.`,
  },
  {
    id: "drill-reset-recover",
    title: "Drill Set A: Reset & Recover Protocol",
    tags: ["drill", "reset", "recover", "baseline", "error", "mid-rally", "composure", "set"],
    content: `Reset and Recover (Drill Set A) directly addresses the most common intermediate error: losing composure in mid-to-long rallies. Session structure (15 min): (1) 5 min — Soft reset drill: partner feeds hard drives, player resets to kitchen with soft controlled drop. (2) 5 min — Rally simulation: play out to shot 15, then initiate reset sequence and rebuild. (3) 5 min — Pressure recovery: partner applies sustained pace, player resets and counter-attacks. Key performance cues: exhale on contact, maintain low center of gravity, step into the reset shot rather than backing away. Recommended frequency: 3 sessions per week for measurable improvement within 3 weeks.`,
  },
];

// Score and return the k most relevant docs for a given player snapshot
function retrieveRelevantDocs(
  stats: { dinkRate: number; kitchenTime: number; volleyRate: number; focusArea: string },
  k = 3
) {
  const scored = KB_DOCS.map(doc => {
    let score = 0;
    const t = doc.tags;
    if (stats.dinkRate < 82 && t.some(x => ["dink","control","kitchen","consistency"].includes(x)))
      score += (82 - stats.dinkRate) * 0.15;
    if (t.some(x => ["rally","fatigue","errors","unforced"].includes(x)))
      score += 2.5; // always relevant given r=0.97
    if (stats.kitchenTime < 45 && t.some(x => ["positioning","court","heatmap","kitchen"].includes(x)))
      score += (45 - stats.kitchenTime) * 0.1;
    if (t.some(x => ["left","vulnerability","corridor"].includes(x)))
      score += 2.0;
    if (stats.volleyRate < 85 && t.some(x => ["volley","technique","reflex"].includes(x)))
      score += (85 - stats.volleyRate) * 0.12;
    if (t.some(x => ["DUPR","rating","progression"].includes(x)))
      score += 1.0;
    if (t.includes("drill") || t.includes("reset"))
      score += 1.5;
    if (stats.focusArea.toLowerCase().includes("left") && t.includes("left"))
      score += 1.5;
    return { doc, score };
  });
  return scored
...
// Direct Claude API call (requires VITE_ANTHROPIC_API_KEY in env)
async function callClaude(userMsg: string, systemMsg: string, maxTokens = 900): Promise<string> {
  const key = (import.meta.env as Record<string, string>).VITE_ANTHROPIC_API_KEY;
